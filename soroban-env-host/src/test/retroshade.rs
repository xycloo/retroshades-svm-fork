use crate::{
    ...
    testutils::AsScVal,
    xdr::{ContractCostType, ContractEventType, ContractId, Hash, ScAddress, ScErrorCode, ScErrorType},
    ContractFunctionSet, Env, Error, ErrorHandler, Host, HostError, Symbol, SymbolSmall, Val, VecObject,
};
use expect_test::expect;
use more_asserts::assert_le;
use soroban_env_common::{EnvBase, I128Small, TryFromVal};
use std::rc::Rc;
...
impl ContractFunctionSet for ContractWithSingleEvent {
    fn call(&self, _func: &Symbol, host: &Host, _args: &[Val]) -> Option<Val> {
        ...
        let mut event_object = host.map_new().unwrap();

        let s = "deposit";
        let target = Symbol::try_from_val(&*host, &s);

        event_object = host
            .map_put(
                event_object,
                target.unwrap().into(),
                I128Small::from_i32(90).into(),
            )
            .unwrap();
        ...
        Some(
            host.zephyr_emit(target.unwrap().into(), event_object.to_val())
                .unwrap()
                .into(),
        )
    }
...
fn contract_event() -> Result<(), HostError> {
    ...
    let retroshades = host.get_retroshades();
    println!("{:?}", retroshades.unwrap());
